...
async def retrieve_symbols(api_key, api_key2):
    ...
    file_symbol_retrieved = Path("/home/chems/Bureau/symbols_retrieved_optimized.txt")
...
async def checked_data(symbols, api_keys):
    ...
    file_symbol_checked = "/home/chems/Bureau/symbols_checked_optimized.txt"
    file_symbol_failed = "/home/chems/Bureau/symbols_failed_optimized.txt"
...
# async def get_asset_finnhub(symbol, index, api_keys):
#     try :
#         key = change_api_key(api_keys, index)
#         async with aiohttp.ClientSession() as session:
#             async with session.get(f'https://finnhub.io/api/v1/stock/financials-reported?symbol={symbol}&token={key}', timeout=180) as response:
#                 data = await response.text()  # Récupère les données en tant que texte
#                 json_data = json.loads(data)  # Convertir le texte en JSON

#                 total_current_assets = None

#                 # Parcourir les données pour trouver la valeur des actifs courants
#                 try:
#                     if json_data['data']:
#                         for item in json_data['data']:
#                             for entry in item['report']['bs']:
#                                 if entry['concept'] == True:
#                                     if entry['concept'] == 'us-gaap_AssetsCurrent':
#                                         total_current_assets = entry['value']
#                                         break
#                 except TypeError:
#                     return 'N/A'
...
def write_csv(data):
    csv_file_path = "/home/chems/Bureau/updated_assets.csv"

    # Création du DataFrame à partir des données nouvelles
    columns = [
        "Symbol", "Company_name", "Price", "Market_Cap", "pe_ratio", "Beta", "Volume_52_weeks", "Volume_1_month",
        "52_Weeks_High", "52_Weeks_Low", "Exchange", "Performance_over_52_weeks", "Country", "CA",
        "Resultat_net", "Sector", "Industry", "Price_52_weeks_Ago", "Currency",
        "Total_assets", "EPS_Annual", "Dividend_per_share_annual", "EBITDA_CAGR", "EBITDA", "ROI_annual", "Ratio_Debt_Equity", 
...
if __name__ == "__main__":
    ...
    #         trouver les valeurs suivante avec d'autre api (example : Google Finance API)
    #         
    #         

    #         1- trouver un certain nombre d'action et les mettre dans une liste (environ 10 000) --> j'en ai trouvé environ 7000, il en faut plus
    #         


    #         2- vérifier que toutes les valuers semblent cohérentes (check si 'N/A' présent dans les valeurs) --> à faire dans le preprocessing des données

    #         


            # Optionnel 
    #         esg_score = 'N/A'  # trouver une api qui donne le score ESG
    #         rating = 'N/A'  # Non disponible directement via Finnhub
    #         total_debt = 'N/A'  # Non disponible directement via Finnhub
